...
- 例如 `/settings` 会匹配 `/settings/user`、`/settings/role` 等所有子路由

### 动态路由缓存

对于带参数的路由（如 `/user/:id`），每个不同的参数都会被缓存为独立的实例：
...
- 可在配置文件中调整 `maxCacheCount`

## 📝 工作原理

1. **路由监听**：监听 `location.pathname` 变化
2. **条件判断**：根据配置判断当前路由是否需要缓存
...
- `app/utils/keepAliveConfig.js` - 配置文件
